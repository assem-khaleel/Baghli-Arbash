...
class ControllerExtensionPaymentMastercardPGS extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting($this->imodule, $this->request->post);
		...
		$data['action'] = $this->url->link($this->imodule_route, 'token=' . $this->session->data['token'], true);
		$data['rehook_events'] = $this->url->link($this->imodule_route . '/hook_events', 'token=' . $this->session->data['token'], true);
	...
	public function transaction_info() {
		...
		$transaction_info = $this->imodule_model->getTransaction($mastercard_pgs_transaction_id);

		if (empty($transaction_info)) {
			$this->response->redirect($this->imodule_route, 'token=' . $this->session->data['token'], true);
		}
		...
		$data['url_void'] = $this->url->link($this->imodule_route . '/void', 'token=' . $this->session->data['token'] . '&mastercard_pgs_transaction_id=' . $transaction_info['mastercard_pgs_transaction_id'], true);
		$data['url_capture'] = $this->url->link($this->imodule_route . '/capture', 'token=' . $this->session->data['token'] . '&mastercard_pgs_transaction_id=' . $transaction_info['mastercard_pgs_transaction_id'], true);
		$data['url_refund'] = $this->url->link($this->imodule_route . '/refund', 'token=' . $this->session->data['token'] . '&mastercard_pgs_transaction_id=' . $transaction_info['mastercard_pgs_transaction_id'], true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link($this->imodule_route . '/transaction_info', 'token=' . $this->session->data['token'] . '&mastercard_pgs_transaction_id=' . $mastercard_pgs_transaction_id, true)
		);
	...
	public function transactions() {
		...
		$transactions_total = $this->imodule_model->getTotalTransactions();
		...
		foreach ($transactions as $transaction) {
			...
			$result['transactions'][] = array(
				...
				'url_info' => $this->url->link($this->imodule_route . '/transaction_info', 'token=' . $this->session->data['token'] . '&mastercard_pgs_transaction_id=' . $transaction['mastercard_pgs_transaction_id'], true)
			);
		}
	...
	public function uninstall() {
		...
		if (version_compare(VERSION, '2.3.0.0', '<')) {
			$this->imodule_model->disableOcmod();

			$redirect_url = $this->url->link($this->imodule_extension_route, 'token=' . $this->session->data['token'], true);
			$success_message = $this->language->get('text_success');

			$this->imodule_model->refreshOcmod($redirect_url, $success_message);
		} else {
			$this->imodule_model->unhookEvents();
		}
	}
	...
	public function hook_events($inline = false) {
		...
		$redirect_url = $this->url->link($this->imodule_route, 'token=' . $this->session->data['token'], true);

		if ($inline) {
			$success_message = $this->language->get('text_success');
		} else {
			$success_message = $this->language->get('text_success_events');
		}

		if (version_compare(VERSION, '2.3.0.0', '<')) {
			$this->imodule_model->disableOcmod();
			$this->imodule_model->enableOcmod();
			$this->imodule_model->refreshOcmod($redirect_url, $success_message);
		} else {
			$this->imodule_model->unhookEvents();
			$this->imodule_model->hookEvents();

			if (!$inline && $this->imodule_model->getModificationId()) {
				$this->imodule_model->deleteOcmod();
				$this->imodule_model->refreshOcmod($redirect_url, $success_message);
			}
		}
	...
	public function capture() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate_capture()) {
			...
			$api_data['partnerSolutionId'] = $this->imodule_model->getPartnerSolutionId();
			...
			if ($new_transaction_id = $this->imodule_model->findTransactionId($transaction_info['order_id'])) {
				$response = $this->imodule_model->api('order/' . $transaction_info['order_id'] . '/transaction/' . $new_transaction_id, $api_data, 'PUT');

				if (!empty($response['result']) && $response['result'] == 'SUCCESS') {
					$this->session->data['success'] = $this->language->get('text_success_capture');

					$this->response->redirect($this->url->link($this->imodule_route, 'token=' . $this->session->data['token'] . '&tab=tab-transaction', true));
		...
		$this->response->redirect($this->url->link($this->imodule_route . '/transaction_info', 'token=' . $this->session->data['token'] . '&mastercard_pgs_transaction_id=' . $mastercard_pgs_transaction_id, true));
	}
	...
	public function delete_tokens($route, $args) {
		if (!empty($args[0]) && $this->user->hasPermission('modify', $this->imodule_route)) {
			$this->load->model($this->imodule_route);
			$this->imodule_model->deleteTokens($args[0]);
		}
	}

	public function order_history_link($route, $args, &$data) {
		if ($this->user->hasPermission('access', $this->imodule_route)) {
			$this->load->model($this->imodule_route);
			$this->imodule_model->orderHistoryLink((int)$args[0], $data);
		}
	}
	...
	protected function validate_capture() {
		...
		$transaction_info = $this->imodule_model->getTransaction($this->request->get['mastercard_pgs_transaction_id']);
