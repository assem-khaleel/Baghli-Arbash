...
class ModelExtensionModule extends Model {
    ...
    /// Addedd from Addmin Extension model for Tips By Jijo on 05-04-2017 Start Here/////
    ...
    public function getModulesByCodeId($code, $id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "module` WHERE `code` = '" . $this->db->escape($code) . "' AND module_id=". $id." ORDER BY `name`");

		return $query->rows;
	}	
    /// Addedd from Addmin Extension model for Tips By Jijo on 05-04-2017 End Here/////
