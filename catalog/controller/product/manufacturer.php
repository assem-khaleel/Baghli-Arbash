		...
		foreach ($results1 as $result1) {
			...
			foreach ($results_products as $result) {
				...
				$categories [] = array(
					...
					'href_category'  => $this->url->link('product/category', 'path=' . $result['category_id'], true),
					'category_name'  => $category_info ,
					'cat_image' => $this->model_tool_image->resize($category_info['image'],318,241)
				);
			}
			...
			if (is_numeric(utf8_substr($result1['name'], 0, 1))) {
				...
				$key = utf8_substr(utf8_strtoupper($result1['name']), 0, 1);
			}
...
if(isset($this->request->post['search'] ) && $this->request->post['search']!= null)
{
	...
	$results=$this->model_catalog_manufacturer->search($search,$manufacturer_id);
			...
			foreach ($results as $result) {
				...
				$data['products'][] = array(
					...
					'category_id'  => $result['category_id'],
                    'thumb'       => $image->resize(['image'],318,241),
					'name'        => $result['name'],
					...
					'href_category'  => $this->url->link('product/category', 'path=' . $result['category_id'], true),
					//('product/category', 'manufacturer_id=' . $result['manufacturer_id'] . '&product_id=' . $result['product_id'] . $url),
...
}else{ $results = $this->model_catalog_product->getProducts($filter_data);
			foreach ($results as $result) {
 ...
 $product_option_value_data['product_id']=$result['product_id'];
			...
			if ($this->config->get('config_review_status')) {
				$data['sorts'][] = array(
					'text'  => $this->language->get('Most rating'),
					'value' => 'rating-DESC',
